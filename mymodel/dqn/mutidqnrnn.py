...
def train_epoch(agent:REMAgent, lr,
    ...
    # for tt in testEnvs:
    for tt in range(1):
        envBestModelPath=os.path.join(store_path,str(tt))
        ...
        f.write('begin to train env '+str(tt)+'\n')
        f.close()
        agentBuffer.refresh()
       # pre-test
                ...
                while surviveFlags>0:
                    ...
                    for testAction,testIndex in zip(testActions,testIndexesList):
                        ...
                        if testTras[testIndex].tras[-1][4]==0:
                            ...
                            testInS[indexDict[testAllNum[testIndex]]][testSceneNum[testIndex]]+=traInfoTest[4]
                            testOutS[indexDict[testAllNum[testIndex]]][testSceneNum[testIndex]]+=traInfoTest[5]
                            testEndInS[indexDict[testAllNum[testIndex]]][testSceneNum[testIndex]]+=traInfoTest[6]
                            testEndOutS[indexDict[testAllNum[testIndex]]][testSceneNum[testIndex]]+=int(traInfoTest[6])^1
                            rewardsTest.append(traInfoTest[7])
                            if traInfoTest[0]>=3:
                                testLenS[indexDict[testAllNum[testIndex]]][testSceneNum[testIndex]]+=traInfoTest[0]
                                testErrorsS[indexDict[testAllNum[testIndex]]][testSceneNum[testIndex]]+=traInfoTest[11]
...
if __name__=='__main__':
    ...
    parser.add_argument('-high',type=int,default=22)
    parser.add_argument('-skipN',type=int,default=15)
