...
def train_epoch(agent:REMAgent, lr,
    ...
    if addPre==True:
        for scene in range(1,5):
            ...
            for env in multienvs:
                ...
                envs.append(DQNRNNEnv(envPath,int(env),scene=int(scene)))
    ...
    for scene in range(1,5):
        ...
        for env in testEnvs:
            ...
            testAllEnvs.append(DQNRNNEnv(testEnvPath,num=int(env),scene=int(scene)))
    ...
    for scene in range(1,5):
        ...
        for env in testEnvs:
            testEnvPath=os.path.join('/home/wu_tian_ci/eyedata/seperate/',env,str(scene))
            testenvs.append(DQNRNNEnv(testEnvPath,num=int(env),scene=int(scene)))
    ...
    for tt in testEnvs:
       ...
       # pre-test
        ...
        for scene in range(1,5):
            ...
            envs.append(DQNRNNEnv(envPath,num=int(tt),scene=int(scene)))
            envFlags.append(False)
            envPaths.append(envPath)
            trajectorys.append(DQNRNNTrajectory2())
        ...
        for j in range(len(testAllEnvs)//3):
            testErrorR='\nerror: '
            testAccR='\nIPA1: '
            testEndAccR='\nIPA2: ' 
            for i in range(1,5):
            ...
            with open(preTestInfos[j],'a',encoding='UTF-8') as f:
                f.write('pretest trainenv: '+tt+'\n+env: '+str(j+17)+'\n'+testErrorR+testAccR+testEndAccR+'\n')

        for filesNum in range(1,6):
            for envI in range(len(envs)):
                if envs[envI].num==int(tt):
                    envs[envI].topN=filesNum
                else:
                    envs[envI].topN=5
                envs[envI].load_dict()
                envs[envI].get_file()
            for K in tqdm(range(epochs)):
                ...
                #scene
                numFlagTrain=len(envs)//3
                numFlagTest=len(testenvs)//3
        ...
        #ans -> state last_goal_state last_goal goal is_end
            ...
            for j in range(len(testAllEnvs)//3):
                ...
                testAccR+='\n'
                testEndAccR+='\n'
                # print (testAccR,testErrorR,testEndAccR,type(testAccR),type(testErrorR),type(testEndAccR))
                with open(preTestInfos[j],'a',encoding='UTF-8') as f:
                    f.write('trainenv: '+tt+'\n+env: '+str(j+np.min(testNum))+' fileNum: '+ str(filesNum)+'\n'+testErrorR+testAccR+testEndAccR)
        for j in range(len(testAllEnvs)//3):
            with open(preTestInfos[j],'a',encoding='UTF-8') as f:
                f.write('end\n')
...
if __name__=='__main__':
    ...
    with open(os.path.join(store_path,'envsinfo.txt'),'w') as f:
        f.write('envs'+str(envs)+'\n trainEnvs:'+str(trainEnvs)+'\n testEnvs:'+str(testEnvs)+'\n'+' lr:'+str(args.lr)+' preload: '+str(args.preload)\
                ...
                +'layers: '+str(args.layers)+'\nactorload: '+actor_load+'\n idFlag '+str(args.idFlag)+' itFlag: '+str(args.itFlag))
        if args.sup!='50':
            f.write('\n'+args.sup)
